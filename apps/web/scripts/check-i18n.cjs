...
const viCatalog = readCatalog("vi-VN");
const enCatalog = readCatalog("en-US");
const viKeys = flattenKeys(viCatalog).sort();
const enKeys = flattenKeys(enCatalog).sort();
const viSet = new Set(viKeys);
const enSet = new Set(enKeys);
const missingInEnglish = viKeys.filter((key) => !enSet.has(key));
const missingInVietnamese = enKeys.filter((key) => !viSet.has(key));
const invalidMessages = [
  ...invalidIcuMessages(viCatalog, "vi-VN"),
...
if (
  ...
  if (missingInVietnamese.length) {
    console.error(`Missing in vi-VN:\n${missingInVietnamese.map((key) => `  ${key}`).join("\n")}`);
  }
  ...
  console.log(`i18n check passed: ${enKeys.length} message keys across 2 locales.`);
}
